...
**The "รวมสัดส่วนงวด" progress bar (`changeCount`'s totalPct indicator, line ~766) animates via `transform: scaleX()`, not `width` (fixed 2026-06-30, `/impeccable audit` finding).** `width: 100%` + `transformOrigin: 'left'` + `transform: scaleX(pct/100)` renders pixel-identical to the old `width: '${pct}%'` version (same rounded-track clipping via the parent's `overflow: hidden`) but doesn't animate a layout property. Automated detector (`detect.mjs`) flagged the old version as a `layout-transition` anti-pattern; confirmed gone on re-scan.
...
- **`Modal.tsx` has real dialog semantics (fixed 2026-06-30):** `role="dialog"`, `aria-modal="true"`, `aria-labelledby` wired to the title, Escape closes it, the dialog box receives focus on open, and focus returns to whatever opened it on close. One shared component, so this covers every modal in the app (Approve/Reject/Cancel/Delete/the resubmit no-change confirm) at once — none of it changes anything visible. **Known limitation, not a bug:** focus-restore-on-close only reliably lands back on the trigger for keyboard-driven opens. `Button.tsx` deliberately calls `.blur()` on `onMouseUp` (to stop the focus-visible ring from sticking after a mouse click — see that file's own comment) — so a mouse-clicked trigger is already unfocused by the time the modal's effect captures `document.activeElement`, and focus ends up on `<body>` after closing. This was already true of every mouse-driven interaction in the app before this fix; it isn't something this change introduced or could meaningfully improve without touching `Button.tsx`'s own deliberate behavior, which was out of scope. Verified via Playwright: keyboard open (`focus()` + Enter) → focus restores correctly; mouse open (`.click()`) → lands on body, matching pre-existing behavior.
- **Text contrast (fixed 2026-06-30):** `#929EB4` (`--color-muted`) was being used as the actual text color in a few places — `StatusBadge`'s subtitle line, the proposal-no subtext in `RequestListPage`'s table, and a few labels inside `DatePicker` — measuring **2.7:1** against white, below WCAG AA's 4.5:1 minimum for body text. Swapped those specific instances to `#586782` (`--color-secondary`, 5.72:1) — an existing token already used as secondary text everywhere else, not a new color. `#929EB4` is still correct and untouched everywhere it's a non-text/decorative use (disabled-input styling per `FormField.tsx`'s `disabledStyle`, which WCAG explicitly exempts).
- `Button.tsx`'s `loading` state now sets `aria-busy` so screen readers get feedback during async actions (no visual change).
- `AppShell.tsx`'s module-switcher row is a `<nav aria-label="เมนูโมดูล">`, not a bare `<div>` — it already functioned as primary navigation.

**Known gaps, confirmed but intentionally not fixed (2026-06-30 audit):** the app has zero responsive breakpoints anywhere except `print.css` and `prefers-reduced-motion` — confirmed via live test that `RequestListPage` overflows horizontally and its table becomes unreadable at 375px. This needs an actual mobile layout decision (table → card collapse, header restructuring), not a contained bug fix, so it was left as the existing open item (§14) rather than retrofitted under a fix-only constraint. Status icon colors (`#FFCC00` pending, `#66C5C5` approved) still measure below the 3:1 non-text-contrast minimum on their own — left as-is since the code's own comment already documents this as a confirmed, deliberate decision (text label is the real carrier of meaning, color is supplementary), not an oversight.
...
3. **Mobile responsiveness** — the app is desktop-only today. The only responsive behavior anywhere is `FieldGrid`'s `auto-fit` column collapse; nothing else adapts below desktop width. **Confirmed with live evidence 2026-06-30** (`/impeccable audit`): at 375px, `RequestListPage` overflows horizontally (`scrollWidth` 631px) and the table becomes unreadable (headers wrap mid-word, request numbers truncate to one character). This needs an actual layout decision (table→card collapse, header restructuring) — flagged but deliberately left unfixed in that round since it requires new UI, not a contained bug fix.
